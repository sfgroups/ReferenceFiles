...
/**
 * Created by sundaram on 2/10/2017.
 */
class SSLINfo{
    String cname
    String sn
...
class Test {
    def static main(args) {
        ...
        Path source = FileSystems.getDefault().getPath("C:\\shared\\work\\mycode\\GroovyCode\\src\\main\\groovy", "main.tpl")
        Path target = FileSystems.getDefault().getPath('C:\\shared\\work\\mycode\\GroovyCode\\out\\production\\classes', 'main.tpl')
        new File(target.toAbsolutePath().toString()).delete()
        Files.copy(source, target)

        def ssl = [ new SSLINfo(cname:'sfgroups.com', sn:'123', expiredate: '12/29/2018'),new SSLINfo(cname:'anu.com', sn:'4576', expiredate: '12/29/2018'),
                    new SSLINfo(cname:'anu.com', sn:'4576', expiredate: '12/29/2018', ticket:'SR456', tikcetstatus: 'Open')]
        ...
        def dir = 'C:\\shared\\work'
        def filename = dir + File.separator + 'email.css'
        def persons = ['Bob', 'Alice']

        def values = ["1", "2", "3"]

        def model = [message: 'It works!', values: values, persons: persons, ssl: ssl]

        Template template = engine.createTemplate("""
    layout 'main.tpl', 
    pageTitle: 'SSL Certificate Information',    
    ...
    actions: contents {
         ...
         tr(){
            th(){ yield "CNAME" }
            th(){ yield "Ser. NO" }
            th(){ yield "Expire Date" }
            th(){ yield "Ticket" }
            th(){ yield "Ticket Status" }
         }
         }
         tbody(){
         ssl.each { c ->
...
// Render output for template.
        ...
        new File('c:\\shared\\work\\a.html').write(result)
